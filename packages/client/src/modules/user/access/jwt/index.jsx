...
const setJWTContext = async operation => {
  ...
  const headers =
    ['login', 'refreshTokens', 'authenticateAdmin'].indexOf(operation.operationName) < 0 && accessToken
    ...
    (async () => {
      ...
      try {
        sub = forward(operation).subscribe({
          next: result => {
            const promise = (async () => {
              if (operation.operationName === 'login' || operation.operationName === 'authenticateAdmin') {
                const { operationName } = operation;
                if (result.data[operationName].tokens && !result.data[operationName].errors) {
                  const {
                    data: {
                      [operationName]: {
