  ...
  options: {
    update: function update(proxy, _ref12) {
      ...
      try {
        ...
        var data = proxy.readQuery({ query: _graphql_Dealers_graphql__WEBPACK_IMPORTED_MODULE_16___default.a, variables: { first: 12, after: 0 } });
        console.log('DATA::', data);
        var _edges = data.dealers.edges;
        //
        // if (_edges.length === 0) {
        //   data.dealers.edges.push({ node: updateDealer, cursor: 0, __typename: 'DealersEdges' });
        // }
        // // data.dealers.edges.push({ node: updateDealer });
        // // data.dealers_count = data.dealers.edges.length;
        proxy.writeQuery({
          query: _graphql_Dealers_graphql__WEBPACK_IMPORTED_MODULE_16___default.a,
          data: babel_runtime_helpers_extends__WEBPACK_IMPORTED_MODULE_3___default()({ dealers: { edges: [].concat(babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_0___default()(data.dealers.edges), [{ node: updateDealer, cursor: 0 }]) } }, data.dealers)
        });
...
//# sourceMappingURL=index.12233fc0d26de028a682.js.map