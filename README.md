...
If deploying for the first time, run this command:

```git remote add deploy git@chf.modsquad.io:CHF-R_Comm.git```

Your .git/config file should have this section now:

```
[remote "deploy"]
	url = git@chf.modsquad.io:CHF-R_Comm.git
	fetch = +refs/heads/*:refs/remotes/deploy/*
```

To push code to production, run ```git push deploy development```

This pushes code to a bare repository on chf.modsquad.io, checks out the development branch, recompiles the code, and restarts the daemon.
